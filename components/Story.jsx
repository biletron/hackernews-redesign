...
export default function Story({ story }) {
  const getHost = (url, id) => {
    url = !url ? `https://node-hnapi.herokuapp.com/item/?id=${id}` : url;
    url = new URL(url);
    return url.hostname;
  };
    ...
    <Link href={`/items/${story.id}`}>
      <div className="news-card flex flex-row font-inter">
        <div className="px-5 hidden lg:flex flex-col justify-center text-center w-20">
          <UpIcon />
          <p className="text-gray-600 text-xs font-medium">{story.points}</p>
        </div>
        <div className="px-5">
          ...
          <div className="flex mt-2">
            ...
            <p className="text-xs text-gray-600 mr-4"> {story.time_ago}</p>
